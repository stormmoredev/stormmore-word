...
<article id="article">
    <header class="flex flex-row justify-between items-center">
        ...
        <div class="text-base text-zinc-400 dark:text-zinc-500">
            <a href="<?php echo url("/b/vote/$post->slug") ?>" class="post-gratitude">
                {{ $post->votes_num}} {{ _ post_gratitude }}
            </a>
            <time>
                <span class="ml-3">{{ $post->published_at | date d m  Y }}</span>
            </time>
...
<template id="write-comment-authorized">
    <form action="{{ url("/b//add-comment/$slug") }}" method="post" class="relative">
        <input name="article-id" type="hidden" value="{{ $post->id }} "/>
